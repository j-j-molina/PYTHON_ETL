...
def cross_validate_model(
    ...
    """
    StratifiedKFold CV sobre train.
    ...
    Args:
        ...
        estimator:  instancia sklearn ya ajustada (se clona internamente).
        X_train:    features de entrenamiento.
        y_train:    target de entrenamiento.
        n_splits:   número de folds.
        scoring:    métrica principal ('recall', 'roc_auc', 'f1').
        fit_params: kwargs adicionales para fit() (ej. sample_weight).

    Returns:
        dict con mean, std y scores por fold.
    """
    from sklearn.base import clone

    skf        = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
    pipe       = Pipeline(steps=[("model", clone(estimator))])
    ...
    logger.info("-" * 60)
    logger.info("CV StratifiedKFold(k=%d) — %s", n_splits, name)
    logger.info("-" * 60)
...
if __name__ == "__main__":
    ...
    for i, (name, _, fit_params) in enumerate(model_definitions):
        cv_r = cross_validate_model(
            ...
            X_train, y_train,
            n_splits=cv_folds, scoring=cv_scoring,
        )
    ...
    # pipeline_base viene de build_features() ya ajustado sobre el dataset
    # completo. Contiene Winsorizar con caps de train e ImputacionSegmentada
    # con medianas de train. Al cargarlo en producción se evita re-fittear
    # sobre datos nuevos (data leakage en caps y medianas).
